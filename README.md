...
Текущая правка: [scenario-router patch](PATCH_REVIEW.md). На символ назначается одна
стратегия до поиска входа; она сохраняет владение до завершения сценария.
Движок отвечает за исполнение, учёт и денежные ограничения. Цель и область входа
после готовности фиксируются, повторный отказ риска не обновляет возможность.

Предыдущее указание автоматически запускать 24-часовой экзамен отменено.
В этой задаче допускаются только локальные тесты и сохранённые события;
новый рыночный запуск требует отдельного решения владельца.

## Навигация

- [Архивный профиль экзамена: 24 часа, состав стратегий, риск и критерий +10% net](docs/paper-exam-24h.md).
- [Capture-профили 1h/12h/24h: обычный UI и полная запись](docs/paper-capture-profiles.md).
...
Текущая задача — [суточный экзамен текущего состава с полной записью](docs/paper-exam-24h.md):

```powershell
.\scripts\run-paper-capture-24h.ps1
```

После проверок открыть http://127.0.0.1:8000/ и нажать Start. Через 24 часа бот закроет paper-позиции и автоматически сохранит запись. Дождаться «Запись сохранена» и выполнить `.\scripts\check-paper-capture.ps1 -Profile 24h` в другом терминале. UI остаётся доступен для просмотра; Ctrl+C только закрывает сервер. Данные сохраняются в новой папке `data/paper-captures/24h-<дата>-<id>`. Итог экзамена и результаты стратегий будут в `capture-check.json`. Общий лимит убытка сессии, как и в текущем часовом профиле, выключен. Старые 1h/12h команды сохраняются, но не являются следующим назначенным прогоном.
