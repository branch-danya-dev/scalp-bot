# Архив HANDOFF перед scenario-router patch

Исторические результаты сохранены без изменения. Указания о следующем запуске отменены актуальным HANDOFF.md.
...
## Текущая задача: суточный экзамен

Владелец выбрал **текущий состав**: breakout/rejection входят, density evidence;
trend и beta выключены. Подготовлен [24h экзамен](docs/paper-exam-24h.md),
`scripts/run-paper-capture-24h.ps1`, проверка `check-paper-capture.ps1 -Profile 24h`.
База перед подготовкой: `b491a1dabc000769b32027e310577f1988ad4e31`.
Цель +10% net от 1000 USDT — критерий результата; риск и торговые правила
совпадают с 1h. Общий session loss gate остаётся выключенным, это явно описано.
Профиль требует 200 GiB. В capture metadata зафиксированы срок/цель; после
полного replay отчёт содержит strategyResults и exam passed/failed/incomplete.
Новые сделки не записывались, прибыльность не доказана. Следующим запуском
назначен 24h, прежние указания на следующий час ниже — исторические.

Проверки подготовки: **995 passed за 145,76 с**; отдельно capture/exam —
26 тестов. Все три launcher-профиля прошли `-Check`; для 24h доступно 796 GiB.
Проверены неизменность торговых параметров относительно 1h, граница +10% без
округления, ранний Stop, неполный срок, нечисловой баланс и статистика стратегии
после полного replay. Реальный суточный запуск ещё не начинался.

## Предыдущий инкремент: первая фиксация и затухание пробоя
